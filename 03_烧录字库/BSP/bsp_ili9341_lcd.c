...
/**
 * @brief  在ILI9341显示器上的一个矩形区域填充像素数组
 */
void ILI9341_FillArrColor(uint16_t xStart,uint16_t yStart,uint16_t xEnd,uint16_t yEnd,uint16_t * colorP){
		uint16_t width,height;
		uint32_t i;
		width = xEnd-xStart+1;
		height = yEnd-yStart+1;
		uint32_t size = width * height;
		ILI9341_OpenWindow ( xStart, yStart, width, height);
		/* memory write */
		ILI9341_Write_Cmd ( CMD_SetPixel );	
			
		for ( i = 0; i < size; i ++ )
			ILI9341_Write_Data ( *(colorP+i) );
}
...
void ILI9341_Clear ( uint16_t usX, uint16_t usY, uint16_t usWidth, uint16_t usHeight )
{
	...
	ILI9341_FillColor ( usWidth * usHeight, 0xFFFF );		
