...
/**
  ...
  * @note   main()已设置NVIC_PriorityGroup_4，此处不再重复设置
  *         优先级7 = FreeRTOS安全（>configLIBRARY_MAX_SYSCALL_INTERRUPT_PRIORITY）
  * @param  无
  * @retval 无
  */
static void ESP8266_USART_NVIC_Configuration(void)
{
    ...
    /* NVIC_PriorityGroupConfig 已在main()中设置，此处省略 */
...
/*
 * 函数名：ESP8266_Cmd
 * 描述  ：向WF-ESP8266模块发送AT指令
 * 输入  ：cmd：发送的指令
 *         reply1、reply2：期待的应答，为NULL时不需要应答，两者为逻辑或关系
 *         waittime：等待应答的时间
 * 输出  : 1：指令发送成功
 *         0：指令发送失败
 * 调用  : 被外部调用
 ...
 * 输入  ：pSSID：WiFi名称字符串
 *        pPassWord：WiFi密码字符串
 *        enunPsdMode：WiFi加密方式
 * 输出  : 1：创建成功
 *         0：创建失败
 * 调用  : 被外部调用
 */
bool ESP8266_BuildAP(char *pSSID, char *pPassWord,
                     ENUM_AP_PsdMode_TypeDef enunPsdMode)
{
    char cCmd[120];
 ...
 * 描述  ：WF-ESP8266模块开启多连接
 * 输入  ：enumEnUnvarnishTx：配置是否开启多连接
 * 输出  : 1：配置成功
 *         0：配置失败
 * 调用  : 被外部调用
 */
 ...
 * 输入  ：enumMode：开启/关闭
 *        pPortNum：服务器端口号字符串
 *        pTimeOver：服务器超时时间字符串，单位秒
 * 输出  : 1：设置成功
 *         0：设置失败
 * 调用  : 被外部调用
 */
bool ESP8266_StartOrShutServer(FunctionalState enumMode, char *pPortNum,
                               char *pTimeOver)
{
 ...
 * 描述  ：获取 WF-ESP8266 的连接状态，适合单端口时使用
 * 输入  ：无
 * 输出  : 2：获得ip
 *         3：建立连接
 *         4：失去连接
 ...
 * 描述  ：获取 WF-ESP8266 的端口（Id）连接状态，适合多端口时使用
 * 输入  ：无
 * 输出  : 端口连接状态，低5位分别对应Id5~0，某位为1则该Id建立连接
 * 调用  : 被外部调用
 */
 ...
 * 输入  ：enumEnUnvarnishTx：配置是否使能透传模式
 *        pStr：要发送的字符串
